...
class DropBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        if not self.training or self.drop_prob == 0.:
            ...
            # 计算中点丢弃率gamma
            feat_size = x.size(-1)
            gamma = self.drop_prob * (feat_size ** 2) / (self.block_size ** 2) / ((feat_size - self.block_size + 1) ** 2)

            FloatTensor = torch.cuda.FloatTensor if x.is_cuda else torch.FloatTensor
            mask = (torch.rand(x.shape) < gamma).type(FloatTensor)

            # 这里利用最大值池化来找到丢弃中点的附近的block_size的所有点，赋值为1
            block_mask = F.max_pool2d(mask, kernel_size=self.block_size, stride=1, padding=self.block_size // 2)

            # 因为padding的原因，需要调整block_mask的size
            if self.block_size % 2 == 0:
                block_mask = block_mask[:, :, :-1, :-1]

            # 因为上面是最大值池化为1，这儿需要反填0
            block_mask = 1 - block_mask

            # 保证做DropBlock 之前和做DropBlock 之后，该层都具有相同的均值
    ...
    def forward(self, x, targets=None, img_dim=None):
        ...
        if targets is None:
            ...
            iou_scores, class_mask, obj_mask, noobj_mask, tx, ty, tw, th, tcls, tconf = build_targets(
                ...
                pred_cls=pred_cls,                  # (b, 3, 13, 13, 80)s in build_targets function]
                anchors=self.scaled_anchors,        # (3, 2) 3个anchor，每个2维
                target=targets,                     # (n_boxes, 6) [detail
